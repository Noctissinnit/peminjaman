...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Barang{{ $motor->mtruid }}</h1>
    </div>
                    ...
                    <form action="{{ route('motor.update',$motor->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="kategori" id="kategori"
                                ...
                                <option value="Berat" @if(old('kategori',$motor->kategori)==='Berat' ) selected @endif>Motor Matik
                                </option>
                                <option value="Ringan" @if(old('kategori',$motor->kategori)==='Ringan' ) selected @endif>Motor Bebek
                                </option>
                            </select>
                            @error('kategori')
                            <small class="form-text text-danger">{{ $message }}</small>
                        ...
                        <button type="submit" class="btn btn-info">Simpan Edit</button>
                    </form>
                </div>
            </div>
        </div>
